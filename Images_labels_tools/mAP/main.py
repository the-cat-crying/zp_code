...
if os.path.exists(IMG_PATH):
    for dir_path, dir_names, files in os.walk(IMG_PATH):
        if not files:
...
def log_average_miss_rate(l_pre, l_rec, num_images):
    """
    ...
    if l_pre.size == 0:
        ...
        m_r = 1
        fpp_i = 0
        return lam_r, m_r, fpp_i

    fpp_i = (1 - l_pre)
    m_r = (1 - l_rec)

    fpp_tmp = np.insert(fpp_i, 0, -1.0)
    mr_tmp = np.insert(m_r, 0, 1.0)

    # Use 9 evenly spaced reference points in log-space
    ref = np.logspace(-2.0, 0.0, num=9)
    for i, ref_i in enumerate(ref):
        # np.where() will always find at least 1 index, since min(ref) = 0.01 and min(fpp_tmp) = -1.0
        j = np.where(fpp_tmp <= ref_i)[-1][-1]
        ref[i] = mr_tmp[j]

    # log(0) is undefined, so we use the np.maximum(1e-10, ref)
    lam_r = math.exp(np.mean(np.log(np.maximum(1e-10, ref))))
...
def voc_ap(v_rec, v_pre):
    """
    --- Official matlab code VOC2012---
    m_rec=[0 ; rec ; 1];
    m_pre=[0 ; prec ; 0];
    for i=numel(m_pre)-1:-1:1
            m_pre(i)=max(m_pre(i),m_pre(i+1));
    end
    i=find(m_rec(2:end)~=m_rec(1:end-1))+1;
    ap=sum((m_rec(i)-m_rec(i-1)).*m_pre(i));
    """
    v_rec.insert(0, 0.0)  # insert 0.0 at begining of list
    v_rec.append(1.0)  # insert 1.0 at end of list
    m_rec = v_rec[:]
    v_pre.insert(0, 0.0)  # insert 0.0 at begining of list
    v_pre.append(0.0)  # insert 0.0 at end of list
    m_pre = v_pre[:]
    """
     This part makes the precision monotonically decreasing
        (goes from the end to the beginning)
        matlab: for i=numel(m_pre)-1:-1:1
                    m_pre(i)=max(m_pre(i),m_pre(i+1));
    """
    # matlab indexes start in 1 but python in 0, so I have to do:
    #     range(start=(len(m_pre) - 2), end=0, step=-1)
    # also the python function range excludes the end, resulting in:
    #     range(start=(len(m_pre) - 2), end=-1, step=-1)
    for i in range(len(m_pre) - 2, -1, -1):
        m_pre[i] = max(m_pre[i], m_pre[i + 1])
    """
     This part creates a list of indexes where the recall changes
        matlab: i=find(m_rec(2:end)~=m_rec(1:end-1))+1;
    """
    i_list = []
    ...
    """
     The Average Precision (AP) is the area under the curve
        ...
        matlab: ap=sum((m_rec(i)-m_rec(i-1)).*m_pre(i));
    """
    ap = 0.0
    for i in i_list:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for idx, detection in enumerate(dr_data):
            ...
            if show_animation:
                ...
                output_img_path = output_files_path + "/images/detections_one_by_one/" + \
